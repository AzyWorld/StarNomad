...
func _ready():
	...
	if targetSelect > 55:
		...
		random.randomize()
		target = get_tree().get_nodes_in_group("target")[random.randi_range(0, len(get_tree().get_nodes_in_group("target"))-1)]
		behavior = get_tree().get_nodes_in_group("target")[len(get_tree().get_nodes_in_group("target"))-1]
		
func _exit_tree():
	r.randomize()
	is_drop = r.randi_range(1, 100)
	if is_drop < loot_shance:
		emit_signal("loot", position)

func _process(_delta):
	if behavior == null:
		behavior = get_tree().get_nodes_in_group("target")[0]
...
func _on_visible_zone_body_entered(body):
	if body.is_in_group("target"):
		...
		if not target.is_in_group("prioritet"):
			target = body
			is_stop = true
			is_player = false
...
func anim():
	if ((speedX != 0 or speedY != 0) and is_stop) and !($AnimationPlayer.current_animation == "spawn"):
		$AnimationPlayer.play("walk")
	elif (not is_stop) or $AnimationPlayer.current_animation == "spawn":
		pass
	else:
